...
export class AddTraineeComponent implements OnInit {
    ...
    this.sheduleService.getBhuList().subscribe(
      (res: any) => {
        if (res) {
          ...
          this.BasicHealthUnit = this.BasicHealthUnit.filter(x => x.name != 'Plus')
  ...
  public dropdownValueChanged = (value, filter) => {
    ...
    if (filter == 'Div') {
      ...
      this.getHfTypes();
      this.traineeList = []
      this.RegisterForm.get('Designation').setValue([]);
      this.RegisterForm.get('healthFacilitiesData').setValue([]);
      this.RegisterForm.get('Trainnee').setValue([]);
    ...
    if (filter === 'hft') {
      ...
      if (this.RegisterForm.controls.BasicHealthUnitType.value == 'BHU 24/7') {
        tempBHU = 'BHU 24';
        this.getHfList(this.RegisterForm.controls.districtsData.value, this.RegisterForm.controls.HfTypesData.value, tempBHU);
      } else {
  ...
  onSubmit() {
    ...
    // || formValues.BasicHealthUnitType.value == ''
    ...
    this.loading = true;
    this.TraineeDto.ScheduleId = this.EventObj.id;
    this.TraineeDto.Trainees = this.selectedTraineeList
    this.subs.add(
      this.registerService.AddTrainee(this.TraineeDto).subscribe((data) => {
        ...
        if (data === true) {
          ...
          this.toastr.error("Trainee already exists to this training", "Error", { closeButton: true });
          this.loading = false
          // contaxt.activeModal.close();
        }
        console.log('-----------------------------------', data)
