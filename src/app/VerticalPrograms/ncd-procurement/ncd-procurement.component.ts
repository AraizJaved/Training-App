...
@Component({
  selector: 'app-ncd-procurement',
  templateUrl: './ncd-procurement.component.html',
  styleUrls: ['./ncd-procurement.component.scss']
})
export class NcdProcurementComponent implements OnInit {
    ...
    this.subs.add(
      this.developmentService.getProcurementList().subscribe(
        (data) => {
          ...
          this.procurementList = this.procurementList.filter(x => x.userDetail == 'Non Communicable Disease')
