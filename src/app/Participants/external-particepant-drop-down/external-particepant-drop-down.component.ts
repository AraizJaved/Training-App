...
@Component({
  selector: 'app-external-particepant-drop-down',
  templateUrl: './external-particepant-drop-down.component.html',
  styleUrls: ['./external-particepant-drop-down.component.scss']
})
export class ExternalParticepantDropDownComponent implements OnInit {
  ...
  AddExternalparticipant() {
    ...
    let id = this.ParticepantList.filter(x => x.id == this.RegisterForm.controls.id.value)[0]?.id;
    debugger
  ...
  OnSubmit() {
    ...
    this.subs.add(
      this.sheduleService.AddExternalParticepantwithScheduleId(this.ExternalParticipants).subscribe((data) => {
        debugger
