...
@Component({
  selector: 'app-sm-super-visor-hrmp-visits',
  templateUrl: './sm-super-visor-hrmp-visits.component.html',
  styleUrls: ['./sm-super-visor-hrmp-visits.component.scss']
})
export class SmSuperVisorHRMPVisitsComponent implements OnInit , OnDestroy , AfterViewInit {

  public smHRMPVisit: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getSMSuperVisorHRMPVisits(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivtSMHRMPVisit", am4charts.XYChart);

        // Add data
        chart.data = this.smHRMPVisit.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.yes,
            value2: tusd.no
...
// Legend
  ...
  private async getSMSuperVisorHRMPVisits(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getSMSuperVisorHRMPVisits(filterDto).toPromise()
    this.smHRMPVisit = data?.data?.formfilled ?? []
  }
