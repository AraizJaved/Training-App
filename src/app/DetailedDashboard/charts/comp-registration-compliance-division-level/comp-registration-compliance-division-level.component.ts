...
@Component({
  selector: 'app-comp-registration-compliance-division-level',
  templateUrl: './comp-registration-compliance-division-level.component.html',
})
export class CompRegistrationComplianceDivisionLevelComponent implements OnInit ,OnDestroy,AfterViewInit {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getRegistrationComplianceDivisionlevel(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("complianceRegistration2", am4charts.XYChart);

        // Add data
        chart.data = this.registrationCompliance.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.areaCoordinator,
            value2: tusd.dco,
...
// Create series
        ...
        this.createSeries("value1", "Area Coordinator", chart);
        this.createSeries("value2", "DCO", chart);
    

// Legend

        let label = chart.chartAndLegendContainer.createChild(am4core.Label);
        label.text = "Division Level Registration Compliance";
        label.align = "center";
  ...
  private async getRegistrationComplianceDivisionlevel(filterDto: FilterDTO): Promise<void> {
    const data = await this.dashboardService.getRegistrationComplianceDivisionlevel(filterDto).toPromise()
