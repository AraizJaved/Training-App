...
@Component({
  selector: 'app-house-hold-cluster-zero-dose-ei-children',
  templateUrl: './house-hold-cluster-zero-dose-ei-children.component.html',
  styleUrls: ['./house-hold-cluster-zero-dose-ei-children.component.scss']
})
export class HouseHoldClusterZeroDoseEIChildrenComponent implements OnInit , OnDestroy,AfterViewInit {


  public hhcZeroDoseEi: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getHouseHoldClusterZeroDoseEIChildren(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("householdEi", am4charts.XYChart);

      // Add data
      chart.data = this.hhcZeroDoseEi.map(tusd => {
        return {
          category: tusd.name,
          value1: tusd.total,
        }
...
// Legend
      ...
      label.text = "Zero Dose EI Children";
      label.align = "center";
  ...
  private async getHouseHoldClusterZeroDoseEIChildren(filterDto : FilterDTO): Promise<void> {
    const data = await this.dashboardService.getHouseHoldClusterZeroDoseEIChildren(filterDto).toPromise()
  
    this.hhcZeroDoseEi = data?.data?.formfilled ?? []
  }
