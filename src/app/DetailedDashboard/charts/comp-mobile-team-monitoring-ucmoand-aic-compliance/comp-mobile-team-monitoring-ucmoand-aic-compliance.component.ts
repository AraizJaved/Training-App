...
@Component({
  selector: 'app-comp-mobile-team-monitoring-ucmoand-aic-compliance',
  templateUrl: './comp-mobile-team-monitoring-ucmoand-aic-compliance.component.html',
})
export class CompMobileTeamMonitoringUCMOandAICComplianceComponent implements OnInit , OnDestroy , AfterViewInit {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getMobileTeamMonitoringUCMOandAICCompliance(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance7", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Mobile Team Compliance UCMO & AIC Wise ";
      label.align = "center";
  ...
  private async getMobileTeamMonitoringUCMOandAICCompliance(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getMobileTeamMonitoringUCMOandAICCompliance(filterDto).toPromise()
