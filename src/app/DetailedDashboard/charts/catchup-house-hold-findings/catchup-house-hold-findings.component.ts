...
@Component({
  selector: 'app-catchup-house-hold-findings',
  templateUrl: './catchup-house-hold-findings.component.html',
})
export class CatchupHouseHoldFindingsComponent implements OnInit ,AfterViewInit,OnDestroy{

  public catchUpData2: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getCatchupHouseHoldFindings(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("catchUpDiv3", am4charts.XYChart);

      // Add data
      chart.data = this.catchUpData2.map(tusd => {
        return {
          category: tusd.name,
          value1: tusd.match,
          value2: tusd.notMatch,
        }
      })
...
// Create series
    ...
    this.createSeries("value1", "Match", chart);
    this.createSeries("value2", "Not Match", chart);
    
    
// Legend

      let label = chart.chartAndLegendContainer.createChild(am4core.Label);
      label.text = "CatchUp House Hold Findings ";
      label.align = "center";
  ...
  private async getCatchupHouseHoldFindings (filterDto : FilterDTO): Promise<void> {
    const data = await this.dashboardService.getCatchupHouseHoldFindings (filterDto).toPromise()
  debugger
    this.catchUpData2 = data?.data?.formfilled ?? []
  }
