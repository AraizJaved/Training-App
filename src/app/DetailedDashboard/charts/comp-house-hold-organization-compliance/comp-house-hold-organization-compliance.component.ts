...
@Component({
  selector: 'app-comp-house-hold-organization-compliance',
  templateUrl: './comp-house-hold-organization-compliance.component.html',
})
export class CompHouseHoldOrganizationComplianceComponent implements OnInit , AfterViewInit,OnDestroy {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getHouseHoldOrganizationCompliance(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance10", am4charts.XYChart);
...
// Legend
      ...
      label.text = "House Hold Compliance Organization Wise ";
      label.align = "center";
  ...
  private async getHouseHoldOrganizationCompliance(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getHouseHoldOrganizationCompliance(filterDto).toPromise()
