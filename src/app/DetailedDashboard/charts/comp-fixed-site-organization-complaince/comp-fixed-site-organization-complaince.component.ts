...
@Component({
  selector: 'app-comp-fixed-site-organization-complaince',
  templateUrl: './comp-fixed-site-organization-complaince.component.html',
})
export class CompFixedSiteOrganizationComplainceComponent implements OnInit , AfterViewInit,OnDestroy {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getMobileTeamOrganizationCompliance(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance8", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Mobile Team Compliance Organization Wise ";
      label.align = "center";
  ...
  private async getMobileTeamOrganizationCompliance(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getMobileTeamOrganizationCompliance(filterDto).toPromise()
