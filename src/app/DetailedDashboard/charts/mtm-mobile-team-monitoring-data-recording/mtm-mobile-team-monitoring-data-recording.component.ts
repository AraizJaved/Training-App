...
@Component({
  selector: 'app-mtm-mobile-team-monitoring-data-recording',
  templateUrl: './mtm-mobile-team-monitoring-data-recording.component.html',
  styleUrls: ['./mtm-mobile-team-monitoring-data-recording.component.scss']
})
export class MtmMobileTeamMonitoringDataRecordingComponent implements OnInit, OnDestroy, AfterViewInit {

  public mtmDataRecording: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getMTMMobileTeamMonitoringDataRecording(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart() {
    this.browserOnly(() => {
      ...
      const chart = am4core.create("chartdivMTMDataRecording", am4charts.XYChart);

      // Add data
      chart.data = this.mtmDataRecording.map(tusd => {
        return {
          category: tusd.name,
          value1: tusd.yes,
          value2: tusd.no,
      ...
      this.createSeries("value1", "Yes", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
      this.createSeries("value2", "No", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);

      // Legend

      let label = chart.chartAndLegendContainer.createChild(am4core.Label);
      label.text = "Correct Data Recording";
      label.align = "center";
  ...
  private async getMTMMobileTeamMonitoringDataRecording(filterDto: FilterDTO): Promise<void> {
    const data = await this.dashboardService.getMTMMobileTeamMonitoringDataRecording(filterDto).toPromise()
    this.mtmDataRecording = data?.data?.formfilled ?? []
  }
