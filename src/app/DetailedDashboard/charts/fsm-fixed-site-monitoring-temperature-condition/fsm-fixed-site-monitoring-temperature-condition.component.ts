...
@Component({
  selector: 'app-fsm-fixed-site-monitoring-temperature-condition',
  templateUrl: './fsm-fixed-site-monitoring-temperature-condition.component.html',
  styleUrls: ['./fsm-fixed-site-monitoring-temperature-condition.component.scss']
})
export class FsmFixedSiteMonitoringTemperatureConditionComponent implements OnInit , OnDestroy , AfterViewInit{


  public FSMTempratureCondition: any[] = []
  ...
  ngOnInit(): void {
    ...
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getFSMFixedSiteMonitoringTemperatureCondition(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart() {
    this.browserOnly(() => {
        ...
        const chart = am4core.create("fsmchartdivtemp", am4charts.XYChart);

        // Add data
        chart.data = this.FSMTempratureCondition.map(ttm => {
          return {
            category: ttm.name,
            value1: ttm.maintained,
            value2: ttm.notMaintained,
...
// Create series
        ...
        this.createSeries("value1", "Maintained", chart,  tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "Not Maintained", chart,  tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);
...
// Legend
  ...
  private async getFSMFixedSiteMonitoringTemperatureCondition(filterDto: FilterDTO): Promise<void> {
    const data = await this.dashboardService.getFSMFixedSiteMonitoringTemperatureCondition(filterDto).toPromise()
    this.FSMTempratureCondition = data?.data?.formfilled ?? []
  }
