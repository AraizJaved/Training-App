...
@Component({
  selector: 'app-comp-teammonitoring-organization-complaince',
  templateUrl: './comp-teammonitoring-organization-complaince.component.html',
})
export class CompTeammonitoringOrganizationComplainceComponent implements OnInit , AfterViewInit,OnDestroy {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getTeammonitoringOrganizationComplaince(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance6", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Transit Team Compliance Organization Wise ";
      label.align = "center";
  ...
  private async getTeammonitoringOrganizationComplaince(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getTeammonitoringOrganizationComplaince(filterDto).toPromise()
