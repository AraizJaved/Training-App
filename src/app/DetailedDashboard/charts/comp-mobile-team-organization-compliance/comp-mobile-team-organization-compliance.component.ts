...
@Component({
  selector: 'app-comp-mobile-team-organization-compliance',
  templateUrl: './comp-mobile-team-organization-compliance.component.html',
})
export class CompMobileTeamOrganizationComplianceComponent implements OnInit , AfterViewInit,OnDestroy{
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getFixedSiteOrganizationComplaince(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance2", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Fixed Site Compliance Organization Wise ";
      label.align = "center";
  ...
  private async getFixedSiteOrganizationComplaince(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getFixedSiteOrganizationComplaince(filterDto).toPromise()
