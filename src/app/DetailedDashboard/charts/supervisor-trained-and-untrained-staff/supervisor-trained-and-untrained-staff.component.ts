...
@Component({
  selector: 'app-supervisor-trained-and-untrained-staff',
  templateUrl: './supervisor-trained-and-untrained-staff.component.html',
  styleUrls: ['./supervisor-trained-and-untrained-staff.component.scss']
})
export class SupervisorTrainedAndUntrainedStaffComponent implements OnInit, OnDestroy, AfterViewInit {

  public supervisorTrainedUnTrainedStaffData: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getSuperVisorTrainedUnTrainedStaff(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivt", am4charts.XYChart);

        // Add data
        chart.data = this.supervisorTrainedUnTrainedStaffData.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.trained,
            value2: tusd.unTrained,
...
// Create series
        ...
        this.createSeries("value1", "Trained", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "Un Trained", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);
       
// Legend

        let label = chart.chartAndLegendContainer.createChild(am4core.Label);
        label.text = "Trained Supervisors";
        label.align = "center";
  ...
  private async getSuperVisorTrainedUnTrainedStaff(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getSuperVisorTrainedUnTrainedStaff(filterDto).toPromise()
    this.supervisorTrainedUnTrainedStaffData = data?.data?.formfilled ?? []
  }
