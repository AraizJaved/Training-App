...
@Component({
  selector: 'app-ucmoaicsupervisor-cluster',
  templateUrl: './ucmoaicsupervisor-cluster.component.html',
  styleUrls: ['./ucmoaicsupervisor-cluster.component.scss']
})
export class UCMOAICSupervisorClusterComponent implements OnInit, OnDestroy, AfterViewInit {

  public ucmoAicSupervisorClusterData: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getUCMOAICSupervisorCluster(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart() {
    ...
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivUcmoAic", am4charts.XYChart);

        // Add data
        chart.data = this.ucmoAicSupervisorClusterData.map(ucmoAic => {
          return {
            category: ucmoAic.name,
            value1: ucmoAic.aiClessthan4,
            value2: ucmoAic.aiCgreaterthanequal4,
            value3: ucmoAic.ucmOlessthan4,
            value4: ucmoAic.ucmOgreaterthan4
          }
        })
...
// Create series
        ...
        this.createSeries("value1", "AIC < 4", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "AIC >= 4", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);
        this.createSeries("value3", "UCMO < 4", chart, tempData[0].value3, this.findByKey(tempFirstData, ['value3', 'category']), tempFirstData.length);
        this.createSeries("value4", "UCMO > 4", chart, tempData[0].value4, this.findByKey(tempFirstData, ['value4', 'category']), tempFirstData.length);


// Legend

        let label = chart.chartAndLegendContainer.createChild(am4core.Label);
        label.text = "UCMO & AIC Team Monitoring";
        label.align = "center";
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    //await this.getUCMOAICSupervisorCluster()
  }
  ...
  private async getUCMOAICSupervisorCluster(filterDto: FilterDTO): Promise<void> {
    const data = await this.dashboardService.getUCMOAICSupervisorCluster(filterDto).toPromise()
    this.ucmoAicSupervisorClusterData = data?.data?.formfilled ?? []
