...
@Component({
  selector: 'app-transit-team-monitoring-vaccine-condition',
  templateUrl: './transit-team-monitoring-vaccine-condition.component.html',
  styleUrls: ['./transit-team-monitoring-vaccine-condition.component.scss']
})
export class TransitTeamMonitoringVaccineConditionComponent implements OnInit, OnDestroy, AfterViewInit {

  public TTMVaccineCondition: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getTransitTeamMonitoringVaccineCondition(filterDto)
      this.generateChart()
    }))
  }
  private generateChart(){
    ...
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivttm3", am4charts.XYChart);

        // Add data
        chart.data = this.TTMVaccineCondition.map(ttm => {
          return {
            category: ttm.name,
            value1: ttm.dry,
            value2: ttm.cool,
...
// Legend
  ...
  private async getTransitTeamMonitoringVaccineCondition(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getTransitTeamMonitoringVaccineCondition(filterDto).toPromise()
    this.TTMVaccineCondition = data?.data?.formfilled ?? []
  }
