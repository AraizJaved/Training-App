...
@Component({
  selector: 'app-mtm-mobile-team-monitoring-ipc-questions',
  templateUrl: './mtm-mobile-team-monitoring-ipc-questions.component.html',
  styleUrls: ['./mtm-mobile-team-monitoring-ipc-questions.component.scss']
})
export class MtmMobileTeamMonitoringIPCQuestionsComponent implements OnInit ,OnDestroy ,AfterViewInit{

  public mtmIPCQuestions: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getMTMMobileTeamMonitoringIPCquestions(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivMTMIPCQusetions", am4charts.XYChart);

        // Add data
        chart.data = this.mtmIPCQuestions.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.goodIPC,
            value2: tusd.poorIPC,
...
// Create series
        ...
        this.createSeries("value1", "Good IPC", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "POOR IPC", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);
  
// Legend

        let label = chart.chartAndLegendContainer.createChild(am4core.Label);
        label.text = " IPC Questions";
        label.align = "center";
  ...
  private async getMTMMobileTeamMonitoringIPCquestions(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getMTMMobileTeamMonitoringIPCquestions(filterDto).toPromise()
    this.mtmIPCQuestions = data?.data?.formfilled ?? []
  }
