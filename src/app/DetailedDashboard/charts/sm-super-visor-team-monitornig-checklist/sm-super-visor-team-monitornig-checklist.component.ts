...
@Component({
  selector: 'app-sm-super-visor-team-monitornig-checklist',
  templateUrl: './sm-super-visor-team-monitornig-checklist.component.html',
  styleUrls: ['./sm-super-visor-team-monitornig-checklist.component.scss']
})
export class SmSuperVisorTeamMonitornigChecklistComponent implements OnInit ,OnDestroy , AfterViewInit{

  public SMTeamMonitoring: any[] = []
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getSMSuperVisorTeamMonitornigchecklist(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivtSMMonitoring", am4charts.XYChart);

        // Add data
        chart.data = this.SMTeamMonitoring.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.correctlyfilled,
            value2: tusd.incorrect,
...
// Create series
        ...
        this.createSeries("value1", "Correctly Filled", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "Incorrect", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);

// Legend

        let label = chart.chartAndLegendContainer.createChild(am4core.Label);
        label.text = "Team Monitoring Checklist";
        label.align = "center";
  ...
  private async getSMSuperVisorTeamMonitornigchecklist(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getSMSuperVisorTeamMonitornigchecklist(filterDto).toPromise()
    this.SMTeamMonitoring = data?.data?.formfilled ?? []
  }
