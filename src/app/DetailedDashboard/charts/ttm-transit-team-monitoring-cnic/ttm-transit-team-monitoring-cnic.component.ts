...
@Component({
  selector: 'app-ttm-transit-team-monitoring-cnic',
  templateUrl: './ttm-transit-team-monitoring-cnic.component.html',
  styleUrls: ['./ttm-transit-team-monitoring-cnic.component.scss']
})
export class TtmTransitTeamMonitoringCNICComponent implements OnInit , OnDestroy , AfterViewInit {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getTTMTransitTeamMonitoringCNIC(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivTTMCNIC", am4charts.XYChart);

        // Add data
        chart.data = this.ttmCNIC.map(tusd => {
          return {
            category: tusd.name,
            value1: tusd.valid,
            value2: tusd.notValid,
...
// Create series
        ...
        this.createSeries("value1", "Valid", chart, tempData[0].value1, this.findByKey(tempFirstData, ['value1', 'category']), tempFirstData.length);
        this.createSeries("value2", "Not Valid", chart, tempData[0].value2, this.findByKey(tempFirstData, ['value2', 'category']), tempFirstData.length);
...
// Legend
  ...
  private async getTTMTransitTeamMonitoringCNIC(filterDto :FilterDTO): Promise<void> {
    const data = await this.dashboardService.getTTMTransitTeamMonitoringCNIC(filterDto).toPromise()
    this.ttmCNIC = data?.data?.formfilled ?? []
  }
