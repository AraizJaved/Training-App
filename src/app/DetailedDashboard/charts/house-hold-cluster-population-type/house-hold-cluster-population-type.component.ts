...
@Component({
  selector: 'app-house-hold-cluster-population-type',
  templateUrl: './house-hold-cluster-population-type.component.html',
  styleUrls: ['./house-hold-cluster-population-type.component.scss']
})
export class HouseHoldClusterPopulationTypeComponent implements OnInit, AfterViewInit {

  public HHCPopulationType: any[] = []
  ...
  ngOnInit(): void {
    ...
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getHouseHoldClusterPopulationType(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart() {
    ...
    this.browserOnly(() => {
        ...
        const chart = am4core.create("chartdivhhc2", am4charts.XYChart);

        const cityWise: any[] = []
        this.HHCPopulationType.forEach(rcd => {
          if (cityWise.find(cw => cw.name === rcd.name)) {
            return
          }

          const cityGrouped = this.HHCPopulationType.filter(a => a.name === rcd.name)
...
// Legend
  ...
  private async getHouseHoldClusterPopulationType(filterDto: FilterDTO): Promise<void> {
    const data = await this.dashboardService.getHouseHoldClusterPopulationType(filterDto).toPromise()
    this.HHCPopulationType = data?.data?.formfilled ?? []
