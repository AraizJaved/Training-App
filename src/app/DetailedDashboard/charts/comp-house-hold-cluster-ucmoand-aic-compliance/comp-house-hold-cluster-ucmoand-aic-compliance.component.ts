...
@Component({
  selector: 'app-comp-house-hold-cluster-ucmoand-aic-compliance',
  templateUrl: './comp-house-hold-cluster-ucmoand-aic-compliance.component.html',
})
export class CompHouseHoldClusterUCMOandAICComplianceComponent implements OnInit , AfterViewInit,OnDestroy{
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getHouseHoldMonitoringUCMOandAICCompliance(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance9", am4charts.XYChart);
...
// Legend
      ...
      label.text = "House Hold Compliance UCMO & AIC Wise ";
      label.align = "center";
  ...
  private async getHouseHoldMonitoringUCMOandAICCompliance(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getHouseHoldMonitoringUCMOandAICCompliance(filterDto).toPromise()
