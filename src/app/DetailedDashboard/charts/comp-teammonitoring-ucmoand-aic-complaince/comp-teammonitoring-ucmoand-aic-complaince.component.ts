...
@Component({
  selector: 'app-comp-teammonitoring-ucmoand-aic-complaince',
  templateUrl: './comp-teammonitoring-ucmoand-aic-complaince.component.html',
})
export class CompTeammonitoringUCMOandAICComplainceComponent implements OnInit , OnDestroy , AfterViewInit {
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getTeammonitoringUCMOandAICComplaince(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance5", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Transit Team Compliance UCMO & AIC Wise ";
      label.align = "center";
  ...
  private async getTeammonitoringUCMOandAICComplaince(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getTeammonitoringUCMOandAICComplaince(filterDto).toPromise()
