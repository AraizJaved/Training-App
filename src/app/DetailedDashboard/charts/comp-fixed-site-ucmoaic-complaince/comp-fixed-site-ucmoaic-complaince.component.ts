...
@Component({
  selector: 'app-comp-fixed-site-ucmoaic-complaince',
  templateUrl: './comp-fixed-site-ucmoaic-complaince.component.html'
})
export class CompFixedSiteUCMOAICComplainceComponent implements OnInit , AfterViewInit,OnDestroy{
  ...
  ngOnInit(): void {
    this.subs.add(this.filterDto.subscribe(async (filterDto: FilterDTO) => {
      ...
      await this.getFixedSiteUCMOAICComplaince(filterDto)
      this.generateChart()
    }))
  }
  ...
  private generateChart(){
    this.browserOnly(() => {
      ...
      const chart = am4core.create("compliance1", am4charts.XYChart);
...
// Legend
      ...
      label.text = "Fixed Site Compliance UCMO & AIC Wise ";
      label.align = "center";
  ...
  private async getFixedSiteUCMOAICComplaince(filterDto :FilterDTO): Promise<void> {
     
    const data = await this.dashboardService.getFixedSiteUCMOAICComplaince(filterDto).toPromise()
