...
@Component({
  selector: 'app-drug-control-procurement',
  templateUrl: './drug-control-procurement.component.html',
  styleUrls: ['./drug-control-procurement.component.scss']
})
export class DrugControlProcurementComponent implements OnInit {
    ...
    this.subs.add(
      this.developmentService.getProcurementList().subscribe(
        (data) => {
          ...
          this.procurementList = this.procurementList.filter(x => x.userDetail == 'CDC')
